...
let rec tcheck (ctx : context) (t : term) (ty : ty) (q : int) : context option =
  let _ = Printf.printf "q(%d)\n" q in
  if not (q = 0 || q = 1) then None
  else
    match t with
    | Var v ->
        let* ty', q' = FMap.find_opt v ctx in
        if eq ty ty' && q' >= q then
    ...
    | App t ->
        ...
        if eq ty ty' then
          ...
          if FMap.for_all (fun _ (_, q) -> q >= 0) ctx' then Some ctx' else None
        else
