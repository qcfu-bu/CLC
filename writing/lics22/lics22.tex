...
\lstset{basicstyle=\scriptsize,language=Coq,showstringspaces=false}
...
\begin{document}
  ...
  Readers familiar with the literature on linear logic or linear type theory will have noticed at this point that CLC does not possess the famous $\otimes$ connective. Though it is entirely possible to encode $\otimes$ in core CLC through a linear Church encoding, proofs and programs written this way will be incredibly unwieldy. 

  A major design goal of CLC is to support an ergonomic interface for users to define inductive types \cite{inductive,cic}. The presence of universe sorts $U$ and $L$ allows users to define inductive types to be non-linear or linear. The lack of ! exponential simplifies the construction and elimination of inductive terms through direct use of constructors and pattern-matching. The $\otimes$ connective becomes a definable inductive type.
  ...
  Due to the complexity of checking soundness for dependent linear type definitions (such as \texttt{FTensor}) and pattern-matching based elimination, we have not verified the algorithm employed by our implementation yet.
