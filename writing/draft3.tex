...
\begin{document}
  ...
  \begin{itemize}
    ...
    \item We observe that CLC is highly backwards compatible with CC$\omega$, enabling us to construct a reduction preserving model of CLC in CC$\omega$. This shows that CLC is consistent. 
    ...
    \item Furthermore, we extend CLC with linear inductive data, demonstrating CLC capabilities for correct by construction imperative programming.
  ...
  \begin{lemma} Non-linear Substitution. \\
    For $\Gamma_1, x \utype A \vdash m : B$ and $\Gamma_2 \vdash n : A$, if $\pure{\Gamma_2}$ and $\mrg{\Gamma_1}{\Gamma_2}{\Gamma}$ are valid for some $\Gamma$, then $\Gamma \vdash m[n/x] : B[n/x]$.
  \end{lemma}

  \begin{lemma} Linear Substitution. \\
    For $\Gamma_1, x \ltype A \vdash m : B$ and $\Gamma_2 \vdash n : A$, if $\mrg{\Gamma_1}{\Gamma_2}{\Gamma}$ is valid for some $\Gamma$, then $\Gamma \vdash m[n/x] : B[n/x]$.
  \end{lemma}
  ...
  \begin{theorem} Propagation. \\
     For any context $\Gamma$, term $m$, type $A$, and sort $s$, if $\Gamma \vdash m : A$ is a valid judgment, then there exists some sort $s$ and level $i$ such that $\overline{\Gamma} \vdash A : s_i$.
    \label{propagation}
  \end{theorem}
  ...
  \begin{theorem} Subject reduction. \\
    For $\Gamma \vdash m : A$, if $m \pstep n$ then $\Gamma \vdash n : A$.
  \end{theorem}
  ...
  \begin{lemma} Narity. \\
    If $\Gamma \vdash m : A$ is a valid judgment, for any $x \notin \Gamma$, there is $occurs(x, m) = 0$.
  \end{lemma}
  ...
  \begin{theorem} Linearity. \\
    If $\Gamma \vdash m : A$ is a valid judgment, for any $(x \ltype B) \in \Gamma$, there is $occurs(x, m) = 1$.
  \end{theorem}

  \subsubsection{Promotion and Dereliction}
  In most presentations of linear logic, there is no clear distinction between linear and non-linear objects. Non-linear objects are created by packing linear objects under non-linear contexts into the ! exponential through the \textit{promotion} rule. Conversely, the \textit{dereliction} rule unpacks ! objects so they can be used in place of linear ones. Linear type theories \cite{abramsky1993,dill,vakar14} based on ! will contain introduction and elimination rules for ! types that serve similar purpose to \textit{promotion} and \textit{dereliction}.

  The linear types of CLC are not designed around packing and unpacking !, so there are no explicit rules for \textit{promotion} and \textit{dereliction}. However, \textit{promotion} and \textit{dereliction} for canonical types are derivable as theorems through $\eta$-expansion.

  \begin{theorem} Promotion \\
    If $\Gamma \vdash m : (x \stype{s} A) \multimap B$ and $\pure{\Gamma}$ are valid judgments, then there exists $n$ such that $\Gamma \vdash n : (x \stype{s} A) \rightarrow B$ is derivable.
  \end{theorem}

  \begin{theorem} Dereliction \\
    If $\Gamma \vdash m : (x \stype{s} A) \rightarrow B$ is a valid judgment, then there exists $n$ such that $\Gamma \vdash n : (x \stype{s} A) \multimap B$ is derivable.
  \end{theorem}
  ...
  \begin{theorem} \label{embedding} Embedding. \\
    If $\Gamma \vdash m \stype{s} A$ is a valid judgment in CLC, then $\erase{\Gamma} \vdash \erase{m} : \erase{A}$ is a valid judgment in CC$\omega$.
  \end{theorem}
