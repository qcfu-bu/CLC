...
\begin{document}
  ...
  In most presentations of linear logic, non-linear objects are created by packing linear objects under non-linear contexts into the ! exponential through the \textit{promotion} rule. Conversely, the \textit{dereliction} rule unpacks non-linear objects so they can be used in place of linear ones. Linear type theories \cite{abramsky1993,dill,vakar14} utilizing exponential for non-linearity will contain ! types and rules that are equivalent to \textit{promotion} and \textit{dereliction}.

  Linear types of CLC are not designed around packing and unpacking !, so there are no rules for \textit{promotion} and \textit{dereliction}. Arbitrary computations existing at the type level also muddle the association between which types can be promoted or derelicted to which. Nevertheless, \textit{promotion} and \textit{dereliction} for canonical types are derivable as theorems through $\eta$-expansion.
  ...
  \begin{lemma} \label{preserve} Erasure preserves reduction. \\
