...
let rec infer_i ictx t = 
  ...
  | G ty ->
    ...
    Type I
  | G_intro t ->
    let ty, lctx = infer_l ictx empty t in
    let () = assert_msg (is_empty lctx) "infer_i G_intro" in
    G ty
...
and infer_l ictx lctx t = 
  ...
  | LetIn (t, b) -> (
    let x, b = unbind b in
    let ty, lctx = infer_l ictx lctx t in
    let () = check_i ictx ty (Type L) in
    let lctx = add x ty lctx in
    let ty, lctx = infer_l ictx lctx b in
    let () = assert_msg (not_in x lctx) "infer_l F_elim" in
    (ty, lctx))
  ...
  | Tensor_elim (t, mb) -> (
    ...
    | Tensor (ty1, ty2) ->
      ...
      let ty, lctx = infer_l ictx lctx b in
      let () = assert_msg (not_in x1 lctx) "infer_l Tensor_elim" in
      let () = assert_msg (not_in x2 lctx) "infer_l Tensor_elim" in
      (ty, lctx)
  ...
  | Unit_elim (t1, t2) -> (
    ...
    match whnf t1 with
    | Unit L -> 
      let t2, lctx = infer_l ictx lctx t2 in
      (t2, lctx)
  ...
  | Axiom (ty, b) ->
    ...
    let lctx = add x ty lctx in
    let ty, lctx = infer_l ictx lctx b in
    let () = assert_msg (not_in x lctx) "infer_l Axiom" in
    (ty, lctx)
...
and check_l ictx lctx t ty =
  ...
  | Lambda b -> (
    ...
    | Lolli (ty1, ty2) ->
      ...
      let () = assert_msg (not_in x lctx) "check_l Lambda" in
      let lctx = remove x lctx in
      lctx
    | Prod (ty, b') ->
