    ...
    | TCons (id, _) -> (
      ...
      | Some mot -> (
        ...
        | (v_ctx) :: v_ctxs -> 
          List.iter 
            (fun v_ctx' ->  
              assert_msg (Context.equal v_ctx v_ctx')  "check Match") v_ctxs;
          let ty' = subst_p (subst mot t) ty' in
          assert_msg (equal ty ty') "check Match";
          v_ctx)
      | _ -> (
        ...
        | [] -> v_ctx
        | (t, v_ctx) :: v_ctxs -> 
          assert_msg (equal t ty) "check Match";
          List.iter 
            (fun (t', v_ctx') ->  
              assert_msg (equal t t')  "check Match";
              assert_msg (Context.equal v_ctx v_ctx')  "check Match") v_ctxs;
          v_ctx))
    | _ -> failwith "infer Match")
  | _ ->
