...
Proof.
  ...
  { have[[Z1[e h1]]|[r1[A1[B1[s1 e]]]]]:=ihA _ _ H5; subst.
    left. exists (Pi Z1 B s r t).
    split; eauto.
    intros; intro. inv H.
    left. exists (Pi (Act r1 A1 B1 s1) B s r t).
    split; eauto.
    intros; intro. inv H. }
  { asimpl in H5.
    have[[Z1[e h1]]|[r1[A1[B1[s1 e]]]]]:=ihB _ _ H5; subst.
    left. exists (Pi A Z1 s r t).
    split; asimpl; eauto.
    intros; intro. inv H.
    left. exists (Pi A (Act r1 A1 B1 s1) s r t).
    split; asimpl; eauto.
    intros; intro. inv H. }
...
Lemma dual_conv Γ X r A B C s t ξ :
  ok Γ ->
  X.[ren ξ] === Act r A B s ->
  Γ ⊢ X : C : t ->
  can_cancel ξ ->
