...
| l_var Γ x A :
  [ x :l A ∈ Γ ] ->
  [ Γ |- Var x :- A ]
| prod_lam Γ n A B r s t l :
  [ Γ ] ->
  [ Γ |- Prod A B s t :- Sort r l ] ->
  [ A +{s} Γ |- n :- B ] ->
  [ Γ |- Lam n :- Prod A B s t ]
| lolli_lam Γ n A B r s t l :
  [ %Γ |- Lolli A B s t :- Sort r l ] ->
  [ A +{s} Γ |- n :- B ] ->
...
Lemma prod_lam_invX Γ n C :
  [ Γ |- Lam n :- C ] -> 
  forall A B s t l, 
    (C <: Prod A B s t /\ [ %(A +{s} Γ) |- B :- Sort t l ]) ->
    [ A +{s} Γ |- n :- B ].
Proof.
  intros.
...
Lemma lolli_lam_invX Γ n C :
  [ Γ |- Lam n :- C ] -> 
  forall A B s t l, 
    (C <: Lolli A B s t /\ [ %(A +{s} Γ) |- B :- Sort t l ]) ->
    [ A +{s} Γ |- n :- B ].
Proof.
  intros.
  dependent induction H; firstorder.
...
Lemma prod_lam_inv Γ n A B r s t l :
  [ %Γ |- Prod A B s t :- Sort r l ] ->
  [ Γ |- Lam n :- Prod A B s t ] -> 
  [ A +{s} Γ |- n :- B ].
Proof.
...
Lemma lolli_lam_inv Γ n A B r s t l :
  [ %Γ |- Lolli A B s t :- Sort r l ] ->
  [ Γ |- Lam n :- Lolli A B s t ] -> 
  [ A +{s} Γ |- n :- B ].
Proof.
...
Proof.
  ...
  - pose proof (merge_pure2 H1 H0).
    ...
    exists s.
    exists x3.
    replace (Sort s x3) with ((Sort s x3).[n/]) by autosubst.
