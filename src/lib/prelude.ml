...
module Prelude = struct
  let raw, (vctx, ictx) =
    let ch = open_in "./lib/prelude.v" in
    PreludeTop.parse_ch ch

  let main = RTop.main

  let tt = (SMap.find "tt" ictx).id

  let ex_intro = (SMap.find "ex_intro" ictx).id

  let sig_intro = (SMap.find "sig_intro" ictx).id

  let tnsr_intro = (SMap.find "tnsr_intro" ictx).id
end
