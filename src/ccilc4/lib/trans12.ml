...
module TCheck2 = struct
  ...
  and infer_tm ctx env eqns map m =
    ...
    | Fix (a, abs) ->
      ...
      let eqns, map = check_tm ctx env eqns map m a in
      (a, eqns, map)
    | Lam _ -> (fst (meta_mk ctx), eqns, map)
    | App (m, n) -> (
    ...
    | Case (m, a, cls) -> failwith "TODO"
    | Absurd -> (fst (meta_mk ctx), eqns, map)
    | Main -> (Type L, eqns, map)
    | Proto -> (Type U, eqns, map)
    | End -> (Proto, eqns, map)
    | Act (r, s1, a, abs) ->
      let s2, eqns, map = infer_sort ctx env eqns map a in
      if s1 = s2 then
        let x, b = unbind_tm abs in
        let ctx = add_v x a ctx in
    ...
    | Send m -> (
      ...
      let a = UMeta.resolve_tm map a in
      match whnf rd_all env a with
      | Ch (r1, Act (r2, _, a, abs)) when r1 <> r2 ->
        let x, b = unbind_tm abs in
    ...
    | Recv (s, m) -> (
      ...
      let a = UMeta.resolve_tm map a in
      match whnf rd_all env a with
      | Ch (r1, Act (r2, s, a, abs)) when r1 = r2 -> (
        let x, b = unbind_tm abs in
        match s with
        | U ->
          let abs = bind_tm x (Ch (r1, b)) in
          (Data (Prelude.sig_d, [ a; Lam (U, abs) ]), eqns, map)
  ...
  and check_tm ctx env eqns map m a =
    ...
    | Lam (s, abs1) -> (
      let a = UMeta.resolve_tm map a in
      match whnf rd_all env a with
      | Pi (t, a, _, abs2) when s = t ->
        let x, m, b = unbind2_tm abs1 abs2 in
        check_tm (add_v x a ctx) env eqns map m b
      | _ -> failwith "check_tm_Lam")
    | Let (m, abs) ->
      let m_ty, eqns, map = infer_tm ctx env eqns map m in
      let m_ty = UMeta.resolve_tm map m_ty in
      let x, n = unbind_tm abs in
      let ctx = add_v x m_ty ctx in
      let env = VMap.add x m env in
      check_tm ctx env eqns map n a
    | Cons (c, ms) -> (
      let a = UMeta.resolve_tm map a in
      match whnf rd_all env a with
      | Data (_, ns) ->
  ...
  and infer_tm ctx env eqns map m =
    ...
    | Ann (a, m) -> (
      ...
      | Let (m, abs) ->
        ...
        let abs = bind_tm x (Ann (a, n)) in
        let m_elab, eqns, map =
          check_tm ctx env eqns map (Let (m, abs)) a_elab
        in
    ...
    | Pi (s, a, impl, abs) ->
      ...
      Syntax2.
        (Type s_elab, Pi (s_elab, a_elab, impl, bind_tm x b_elab), eqns, map)
    | Fun (a_opt, cls) -> (
      match a_opt with
      | Some a ->
        let _, a_elab, eqns, map = infer_sort ctx env eqns map a in
    ...
    | Match (ms, cls) ->
      ...
      Syntax2.
        (UMeta.resolve_tm map meta, mkApps ct (List.rev ms_elab), eqns, map)
    | If (m, tt, ff) -> (
      let a, m_elab, eqns, map = infer_tm ctx env eqns map m in
      let a = UMeta.resolve_tm map a in
      match whnf rd_all env a with
    ...
    | Close m -> (
      ...
      | _ -> failwith "infer_tm_Close")
    | Embed m ->
      let a, eqns, map = TCheck2.infer_tm ctx env eqns map m in
      (a, m, eqns, map)
