...
and check_prbm ctx env eqns map prbm a =
  ...
  let _ =
    pr "@[<v 0>check_prbm(@;<1 2>@[%a@]@;<1 2>: %a)@]@." UVar.pp_prbm prbm pp_tm
      a
  in
  ...
  | [] -> (
    if has_failed (fun () -> UVar.unify prbm.global) then
      ...
      | _ ->
        let _ = pr "%a@." UVar.pp_prbm prbm in
...
and prbm_add ctx env prbm x a =
  let _ = pr "prbm_add(%a)@." UVar.pp_prbm prbm in
  ...
  and ps_of_tl ps tl =
    ...
    | TBind (_, impl, abs) -> (
      ...
      match (ps, impl) with
      | _, true -> PVar (V.blank ()) :: ps_of_tl ps tl
...
and prbm_subst ctx x prbm m =
  let _ = pr "prbm_subst(%a, %a, %a)@." V.pp x UVar.pp_prbm prbm pp_tm m in
  match prbm.clause with
  | [] -> prbm
  | (es, ps, rhs) :: clause -> (
